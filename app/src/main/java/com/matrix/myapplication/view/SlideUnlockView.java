...
public class SlideUnlockView extends View {
	...
	public SlideUnlockView(Context context, AttributeSet attrs, int defStyle) {
		...
		currentState = STATE_LOCK;
		// 命名空间
		String namespace = "slideunlockdemo";

		// 取出自定义属性中背景图片
		int slideUnlockBackgroundResource = attrs.getAttributeResourceValue(R.drawable.jiesuo_bg, -1);
		// 取出自定义属性中滑块图片
		int slideUnlockBlockResource = attrs.getAttributeResourceValue(R.drawable.jiesuo_button, -1);
		// 取出自定义属性中当前状态
		// 如果解锁状态是true，说明已经解锁
		/**
		 * 当取出自定义属性的背景时，设置背景
